...
export class CartController {
  ...
  @Post('change_size')
  async changeSizeCart(@Headers('cookie') cookie, @Res() res, @Body('cartId') cartId: number, @Body('itemSize') itemSize: string,) {
    try {
      const decodeData = await this.userService.decodeToken(cookie);
      if (!decodeData) {
        throw new Error('유효한 사용자를 찾을 수 없습니다.');
      ...
      const user = await this.userService.findByEmail(decodeData.user.email)
      const result = await this.cartService.changeSizeCart(user.u_id, cartId, itemSize);
      res.send(result);
    } catch (error) {
      console.error("deleteCartOneButton Error:", error);
      res.status(500).send({
  ...
  async changeQuantityCart(@Headers('cookie') cookie, @Res() res, @Body('cartId') cartId: number, @Body('itemQuantity') itemQuantity: string,) {
    try {
      const decodeData = await this.userService.decodeToken(cookie);
      if (!decodeData) {
        throw new Error('유효한 사용자를 찾을 수 없습니다.');
      ...
      const user = await this.userService.findByEmail(decodeData.user.email)
      const result = await this.cartService.changeQuantityCart(user.u_id, cartId, itemQuantity);
      res.send(result);
    } catch (error) {
      console.error("deleteCartOneButton Error:", error);
      res.status(500).send({
